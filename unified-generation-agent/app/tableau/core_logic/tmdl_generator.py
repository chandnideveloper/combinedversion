...
class TmdlGenerator:
    ...
    def _map_data_type(self, json_type: str) -> str:
        ...
        jt = str(json_type or 'string').lower()
        ...
        if 'date' in jt or 'time' in jt: return 'dateTime'
        return 'string'
    ...
    def _clean_dax(self, formula: str, is_parameter: bool = False, is_measure: bool = False, 
        ...
        # Ensure AGG is stripped from ALL expressions. Recursive check for nested AGG.
        while "AGG(" in clean.upper():
            match = re.search(r'(?i)\bAGG\s*\((.*)\)', clean)
            if match:
                inner_content = match.group(1).strip()
                # If formula starts with AGG( and ends with ), strip safely
                if clean.upper().startswith("AGG(") and clean.endswith(")"):
                    clean = clean[4:-1].strip()
                else:
                    # Replace only the AGG(...) part with its content
                    clean = re.sub(r'(?i)\bAGG\s*\((.*?)\)', r'\1', clean)
            else:
                break

        # 9. Rule: USERNAME() / USERPRINCIPALNAME() / CUSTOMDATA() validation
        # These are measure-only functions. If used in a column context, it will fail in Fabric.
        ...
        if display_to_qualified:
            # We must replace in REVERSE order of index to avoid offset corruption
            # OR we collect matches and then replace all at once.
            matches = []
            ...
            for m in tokens:
                ...
                if t_upper in display_to_qualified:
                    start_idx = m.start()
                    # Skip if already part of a qualified name (preceded by ')
        ...
        # Wrap 'Table'[Column] in MAX() if it's standalone/not inside a function
        if is_measure:
            # Match fully qualified columns not preceded by standard aggregation functions
            # Simplified: Find any 'T'[C] and check if it's "raw"
            def wrap_raw_columns(formula_str: str) -> str:
                # Find all 'Table'[Column]
                col_pattern = r"'([^']+)'\[([^\]]+)\]"
                # Functions that accept tables/columns directly
                ...
                def repl(match):
                    ...
                    start = match.start()
                    # Check prefix for function call
                    ...
                    if any(prefix.endswith(f + "(") for f in safe_funcs) or any(prefix.endswith(f + " (") for f in safe_funcs):
                        return full_ref
                    # Also check if it's part of a comparison like IF('T'[C]=... or similar
                    # If it's a measure context, wrapping with MAX is almost always safer
    ...
    def _build_datasource_map(self, api_data: Dict) -> Dict[str, Dict]:
        ...
        try:
            ...
            log_info(f"[TMDL DEBUG] datasources raw type: {type(ds_raw).__name__}, length: {len(ds_raw) if isinstance(ds_raw, (list, dict)) else 'N/A'}")
            global_mode = ''
            if isinstance(ds_raw, dict): 
                global_mode = ds_raw.get('mode', '').lower()
                ds_list = ds_raw.get('datasources', [])
                log_info(f"[TMDL DEBUG] datasources is dict, global_mode='{global_mode}'")
            else:
                ds_list = ds_raw
                log_info(f"[TMDL DEBUG] datasources is list with {len(ds_list)} items")
            ...
            for ds in ds_list:
                ...
                info['mode'] = str(raw_mode).lower() if raw_mode else global_mode
                info['connections'] = connections
                log_info(f"[TMDL DEBUG] DS id='{ds_id}' raw_mode='{raw_mode}' resolved_mode='{info['mode']}'")
    ...
    def _generate_relationships_tmdl(self, relationships: List[Dict], valid_tables: List[str]) -> str:
        ...
        for rel in relationships:
            try:
                ...
                rel_name = rel.get('_override_rel_id') or str(uuid.uuid4())
                lines.append(f"relationship {rel_name}")

                if rel.get('joinOnDateBehavior'):
                    lines.append(f"    joinOnDateBehavior: {rel.get('joinOnDateBehavior')}")
    ...
    def _generate_table_tmdl(self, table_def: Dict, connection_info: Dict, 
        ...
        for col in columns:
            ...
            # columns — ignore any dax_formula they may carry so they emit
            # as plain sourceColumn definitions, not calculated columns.
        ...
        if actual_calcs:
            for calc in actual_calcs:
                ...
                if formula and '=' in formula and not formula.lstrip().upper().startswith('VAR '):
                    ...
                    # Extract name from brackets if present: 'Table'[Col] or [Col]
                    match = re.match(r"(?:'?[^'\[\]\n]+'?)?\[([^\]]+)\]", left_side)
                    if match:
        ...
        if (mode == 'extract' or is_direct_lake) and lakehouse_info:
            ...
            lines.append(f"    partition '{t_name}' = entity")
            lines.append(f"        mode: directLake")
            lines.append(f"        source")
            lines.append(f"            entityName: {t_name}")
            if lh_schema:
                lines.append(f"            schemaName: {lh_schema}")
            else:
                log_error(f"TMDL Core Partition Error: No schema found for DirectLake table '{t_name}'")
            ...
            if conn_type in ['postgresql', 'postgres']:
                ...
                gcp_project = connection_info.get('google_cloud_project_id') or '<gcp-project-id>'
                schema = raw_schema if raw_schema else 'default'
                ...
                if custom_query:
                    safe_query = re.sub(r'--.*', '', custom_query)
                    safe_query = safe_query.replace('"', '""').replace('\n', ' ')
                    m_lines.append(f'    Custom_SQL = Value.NativeQuery(#"Navigation 2", "{safe_query}", null, [EnableFolding=true]),')
                    step_name = "Custom_SQL"
                else:
                ...
                if raw_names:
                    ...
                    if type_items:
                        ...
                        final_step_name = '#"Changed column type"'
                    else:
                        final_step_name = '#"Expanded Column1"'
                else:
                    final_step_name = '#"Converted to table"'
    ...
    def _generate_calculation_group_tmdl(self, group_name: str, calc_items: List[Dict]) -> str:
        ...
        for item in calc_items:
            ...
            raw_dax = item.get('dax_formula') or item.get('powerbi', {}).get('dax') or item.get('powerbi_formula') or item.get('dax')
            # Extract display-to-qualified logic for calc group formulas if available
    ...
    async def generate_tmdl_structure(self, api_data: Dict, folder_path: str, app_name: str,
                                       is_direct_lake: bool = False, direct_lake_schema: str = None):
        failed_files = []
        created_files = []
        ...
        try:
            ...
            from app.tableau.core.logging_utils import _project_id_ctx, _workbook_id_ctx, _run_id_ctx, get_log_token
            ...
            # Register calculation group table names so model.tmdl includes ref tables
            dynamic_calc_groups = []
            ...
            log_info(f"[TMDL DEBUG] datasource_map keys: {list(datasource_map.keys())}")
            for ds_key, ds_info in datasource_map.items():
                log_info(f"[TMDL DEBUG] datasource_map['{ds_key}'] mode='{ds_info.get('mode')}' type='{ds_info.get('type')}'")
            extract_mode = any(info.get('mode') == 'extract' for info in datasource_map.values())
            log_info(f"[TMDL DEBUG] extract_mode={extract_mode}")
            ...
            if isinstance(dl_results, dict):
                ...
                if proc_summary and len(proc_summary) > 0:
                    ...
                    if isinstance(lakehouse_info, dict):
                        ...
                        log_info(f"[TMDL] Resolved lakehouse schema in Core: {lakehouse_info.get('schema')}")
            ...
            if extract_mode:
                ...
                if not isinstance(dl_payload, dict):
                    dl_payload = {}
                processing_summary = dl_payload.get('processing_summary', [])
                
                lakehouse_dict = {}
                workspace_id = "<workspace-id>"
                
                if isinstance(processing_summary, list) and processing_summary:
                    fa = processing_summary[0].get('fabric_artifacts', {})
                    if isinstance(fa, dict):
                        lakehouse_dict = fa.get('lakehouse', {})
                        job_url = fa.get('notebook_job', {}).get('url', '')
                        if job_url:
                            # e.g. https://api.fabric.microsoft.com/v1/workspaces/1f2736f8.../items/...
                            match = re.search(r'/workspaces/([a-zA-Z0-9-]+)/', job_url)
                            if match:
                ...
                if lakehouse_dict:
                    ...
                    if isinstance(processing_summary, list) and processing_summary:
                        ...
                        # Also check bronze_tables if parquet_files is empty
                    ...
                    try:
                        ...
                        if not expr_ok:
                            log_error(f"Failed to push required file: {expr_path}")
                        else:
                            created_files.append(expr_path)
                    except Exception as e:
                        log_error(f"Failed to generate expressions.tmdl: {e}")
            ...
            for tbl in all_tables:
                ...
                # Build PK identification for CTD mapping
            ...
            for cg_name, calc_items in dynamic_calc_groups:
                ...
                cg_ok = await self.file_agent.create_or_update_file(cg_path, cg_content, f"Create calculation group {cg_name}")
                if not cg_ok:
                    failed_files.append(cg_path)
                    log_error(f"Failed to push calculation group file: {cg_path}")
                    return {"success": False, "message": f"Failed to create calculation group: {cg_name}.tmdl", "failed_files": failed_files}
